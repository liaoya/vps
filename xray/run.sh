...
function print_usage() {
    ...
    cat <<EOF
Usage: $(basename "${BASH_SOURCE[0]}") OPTIONS <clean|restart|start|stop> <${_candidate}> <shadowsocks|vmess|vless>
    -h, show the help
    -v, verbose mode
...
fi [[ $# -ne 3 && ${1} != clean ]]; then
    echo "The second and third parameter is required for ${1}"
    exit 1
fi
...
if [[ ${3} != shadowsocks && ${3} != vmess && ${3} != vless ]]; then
    print_usage
    exit 1
fi
...
if [[ ${1} == clean ]]; then
    ...
    _delete_firewall_port "${XRAY[PORT]}" "${XRAY[MKCP_PORT]}"
elif [[ ${1} == restart ]]; then
    docker-compose -f "${ROOT_DIR}/${2}/docker-compose.yaml" restart
    if [[ ${2} == server ]]; then
        _add_firewall_port "${XRAY[PORT]}" "${XRAY[MKCP_PORT]}"
    fi
elif [[ ${1} == start ]]; then
